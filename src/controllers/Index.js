...
class Index extends Controller {
	...
	discord(bot, req, res) {
		...
		res.locals.pagetitle = 'Join RNet on Discord';
		res.locals.content = `If you're not redirected, <a href="${res.locals.redirectURI}" title="Join RNet on Discord">click here</a> to be taken to the RNet Discord server.`;
	...
	upgrade(bot, req, res) {
		if (!req.session.user) {
			...
			res.locals.pagetitle = 'Upgrade RNet';
			res.locals.content = `If you're not redirected, <a href="${res.locals.redirectURI}" title="Upgrade RNet">click here</a> to login first.`;
			return res.render('redirect', { layout: 'redirect' });
		}
