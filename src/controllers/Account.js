...
class Account extends Controller {
	...
	async checkServersWithinSubscription(userId, premiumUser) {
		...
		let rnetPremiumSubscriptions = premiumUser.subscriptions.filter(subscription => subscription.planId.startsWith('premium-'));
		...
		if (activatedGuilds.length > rnetPremiumServers) {
			let removal = activatedGuilds.length - rnetPremiumServers;
		...
		try {
			...
			switch (notification.kind) {
				...
				case braintree.WebhookNotification.Kind.SubscriptionExpired:
					...
					const rnetPremiumSubscriptions = premiumUser.subscriptions.filter(subscription => subscription.planId.startsWith('premium-'));
					if (subscription.premiumUserId && !rnetPremiumSubscriptions.length) {
						try {
	...
	async activate(bot, req, res) {
		...
		if (guildsCount >= rnetPremiumServers) {
